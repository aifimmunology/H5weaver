...
add_cell_ids <- function(h5_list,
    ...
    if(replace_barcode) {
      if(retain_original_barcode) {
        h5_list <- set_list_path(h5_list,
                                 "/matrix/observations/original_barcodes",
                                 sub("-1","",h5_list$matrix$barcodes))
      }
      ...
      h5_list <- set_list_path(h5_list,
                               "/matrix/observations/cell_uuid",
                               ids::uuid(n = length(h5_list$matrix$barcodes),
                                         drop_hyphens = TRUE,
                                         use_time = TRUE))
    }
  ...
  if(add_name) {
    h5_list <- set_list_path(h5_list,
                             "/matrix/observations/cell_name",
                             ids::adjective_animal(n = length(h5_list$matrix$barcodes),
                                                   n_adjectives = 2,
                                                   max_len = 10))
  }
