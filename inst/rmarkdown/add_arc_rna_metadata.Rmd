...
if(is.null(params$in_h5)) {
  ...
  in_pbm <- params$in_pbm
  in_key <- params$in_key
  in_well <- params$in_well
  out_dir <- params$out_dir
}

in_well <- sub("-[AHMR]P","-P",in_well)
...
stm(paste0("IN  Per Barcode Metrics : ", in_pbm))
...
print(c(
  ...
  paste0("IN  Per Barcode Metrics : ", in_pbm),
...
if(!file.exists(in_pbm)) {
  stm(paste0("ERROR: Cannot find IN Per Barcode Metrics file:", in_pbm))
  stop()
}
...
if(! "cell_uuid" %in% names(pbm)) {
  stop("A cell_uuid column is required for arc processing. See 00_arc_uuid.sh in tenx-atacseq-pipeline.")
}

# filter and sort per_barcode_metrics to match h5_list
pbm$barcode <- sub("-1","",pbm$barcode)
pbm <- pbm[match(h5_list$matrix$observations$original_barcode, pbm$barcode),]

# Replace barcodes with joint UUIDs
h5_list <- set_list_path(h5_list,
                         "/matrix/barcodes",
                         unlist(pbm$cell_uuid))
...
if(sum(feature_types != "Gene Expression") > 0) {
  ...
  atk <- feature_list[["_all_tag_keys_"]]
  feature_list[names(feature_list) == "_all_tag_keys_"] <- NULL
  feature_df <- as.data.frame(feature_list)
  ...
  h5_list$matrix$features[["_all_tag_keys_"]] <- atk
