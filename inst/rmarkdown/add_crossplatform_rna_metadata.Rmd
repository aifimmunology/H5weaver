...
if(is.null(in_tenx)) {
  ...
  if(file.exists(in_in)) {
    stm(paste("Found arc_singlecell.csv file:",arc_sin))
...
if(run_type == "arc") {
  ...
  if(! "original_barcodes" %in% names(sc)) {
  ...
  sc <- sc[match(h5_list$matrix$observations$original_barcode, sc$original_barcode),]
