...
if(is.null(params$in_h5)) {
  ...
  in_ss <- system.file("reference/SampleSheet_fallback.csv", package = "H5weaver")
  in_well <- "B000-P0C0W0"
  out_dir <- tempdir()
} else {
  in_h5 <- params$in_h5
...
stm(paste0("IN  SampleSheet     : ", in_ss))
...
if("HashTag" %in% names(ss)) {
  ...
  pool_id <- sub("C.+","",well_id)
  pool_id <- sub("-[A|H|R]P","-P", well_id)
