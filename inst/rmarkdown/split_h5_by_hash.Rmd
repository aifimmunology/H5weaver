...
```{r Chip Pool Batch}
h5_list <- lapply(h5_list,
                  function(x) {
                    x$matrix$observations$chip_id <- sub("W[0-9]+$","",x$matrix$observations$well_id)
                    x$matrix$observations$pool_id <- sub("C[0-9]+$","",x$matrix$observations$chip_id)
                    x$matrix$observations$batch_id <- sub("_RP[0-9]+$","",x$matrix$observations$pool_id)
                    x
                  })
```
