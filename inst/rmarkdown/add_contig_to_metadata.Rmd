---
title: "Add 5 Prime Data to H5 Metadata"
author:
 - Qiuyu Gong 
...
stm(paste0("Please provide input h5"))
  ...
  in_airr <- params$in_airr  
  in_fasta <- params$in_fasta  
...
stm(paste0("IN Airr   : ", in_airr))
stm(paste0("IN Contig    : ", in_contig))
stm(paste0("IN Fasta   : ", in_fasta))
stm(paste0("OUT Dir        : ", out_dir))
stm(paste0("Batch ID        : ", in_batch))
print(paste0("IN H5        : ", in_h5))
print(paste0("IN Contig    : ", in_contig))
print(paste0("IN Airr    : ", in_airr))
print(paste0("IN Fasta    : ", in_fasta))
...
#### Load Contig/Airr/Fasta Files
```{r Load Contig/Airr/Fasta}

stm(paste0("Loading Contig from ", in_contig))
contig<-read.csv(in_contig)

stm(paste0("Loading Airr from ", in_airr))
airr<-read.csv(in_airr)

stm(paste0("Loading Fasta from ", in_fasta))
fasta<-read.csv(in_fasta,header=FALSE)
...
stm("Hashed regrouping Clonotype ")
...
stm("Non-Hashed Skipping regrouping")
...
cellUUId$Well_ID<-cellUUId$well_id
...
meta_info<-new_contig[c('original_barcodes','well_id',
             'contig_id','raw_clonotype_id','raw_consensus_id')]
meta_info<-new_contig[c('original_barcodes','barcode','well_id',
             'contig_id','raw_clonotype_id','raw_consensus_id')]
meta_info$cell_id<-paste0(meta_info$original_barcodes,'-1')
meta_info$fasta_key<-paste0('>',meta_info$cell_id,str_sub(meta_info$contig_id,33,44),"-",meta_info$well_id)
meta_info$fasta_value<-paste0('>',meta_info$barcode,str_sub(meta_info$contig_id,33,44),"-",meta_info$well_id)
meta_info$sequence_id<-paste0(meta_info$original_barcodes,"-1",str_sub(meta_info$contig_id,33,44))
...
new_contig<-new_contig[1:35]
write.csv(new_contig,paste0(out_dir,"/",unique(so[[]]$pool_id),"_",unique(cellUUId$pbmc_sample_id),"_filtered_contig_",in_category,".csv"),row.names=FALSE)

###Airr
airr<-left_join(airr,meta_info,by=c('sequence_id','Well_ID'="well_id"))
airr$cell_id.x<-NULL
airr$cell_id<-airr$barcode
airr$clone_id<-airr$raw_clonotype_id
airr$sequence_id<-airr$contig_id
write.table(airr[c(42,1:33)], file=paste0(out_dir,"/",unique(so[[]]$pool_id),"_",unique(cellUUId$pbmc_sample_id),"_airr_rearrangement_",in_category,".tsv"), quote=FALSE, sep='\t', row.names = F)
###Fasta
replacement_dict <- setNames( meta_info$fasta_value,meta_info$fasta_key)
v_new <- sapply(pull(fasta), function(x) if (x %in% names(replacement_dict)) str_sub(replacement_dict[[x]][1],1,42) else x)
names(v_new)<-NULL
write(v_new, paste0(out_dir,"/",unique(so[[]]$pool_id),"_",unique(cellUUId$pbmc_sample_id),"_filtered_contig_",in_category,".fasta"), sep = "\n", ncolumns = 1)
}
```
...
reformated_contig_df<-reformated_contig_df %>% dplyr::select(contains(in_category))
cols<-colnames(reformated_contig_df)
#write.csv(contig,paste0(out_dir,"/",unique(so[[]]$pool_id),"_",sample_id,"_filtered_contig_",in_category,".csv"),row.names=FALSE)
...
write_h5_list(h5_list,
              h5_file = paste0(out_dir,"/",unique(so[[]]$pool_id),"_",unique(so[[]]$pbmc_sample_id),".h5"),
              overwrite = TRUE)
}else {

write_h5_list(h5_list,
              h5_file = paste0(out_dir,"/",unique(so[[]]$pool_id),"_",sample_id,".h5"),
              overwrite = TRUE)
...
Chord_df<-contig    %>% group_by(v_gene,j_gene) %>%
        tally()%>% as.data.frame()
