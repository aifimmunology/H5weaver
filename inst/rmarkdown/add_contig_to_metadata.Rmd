...
###Airr
airr<-left_join(airr,meta_info)
airr$cell_id<-airr$barcode
airr$clone_id<-airr$raw_clonotype_id
airr$sequence_id<-airr$contig_id
write.table(airr[1:33], file=paste0(out_dir,"/",unique(so[[]]$pool_id),"_",unique(cellUUId$pbmc_sample_id),"_airr_rearrangement_",in_category,".tsv"), quote=FALSE, sep='\t', row.names = F)
###Fasta
